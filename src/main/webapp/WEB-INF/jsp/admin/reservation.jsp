...
<body>
    ...
    <script>
        ...
        function loadReservations() {
            fetch('/reservations', { headers: authHeader() })
                .then(r => r.json())
                .then(result => {
                    if (!result.ok) {
                    ...
                    document.getElementById('reservationsList').innerHTML = `
                        <table>
                            <thead><tr><th>ID</th><th>예약자</th><th>날짜</th><th>시간</th><th>테마</th><th>생성일시</th><th>관리</th></tr></thead>
                            <tbody>
                                ${reservations.map(r => `
                                    <tr>
                                        <td>${r.id}</td>
        ...
        function deleteReservation(id) {
            ...
            fetch('/reservations/' + id, { method: 'DELETE', headers: authHeader() })
                .then(r => r.json())
                .then(result => {
                    if (result.ok) { alert('예약이 삭제되었습니다.'); loadReservations(); }
